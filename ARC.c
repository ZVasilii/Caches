...
void replace(int *p, long long int page_name, struct list* T1, struct list* T2, struct list* B1, struct list* B2)
{
	struct cell* page_in_B2 = find_cell(B2, page_name);
	if (((T1->size) >=1) && (((page_in_B2 != NULL) && ((T1->size) == (*p))) || ((T1->size) >= (*p))))
	{
...
struct cell* fast_get_page(long long int page_name, struct list* T1, struct list* T2, struct list* B1, struct list* B2, int *p)			//main function of ARC
{									
	...
	if ((page_in_T1 != NULL) || (page_in_T2 != NULL))				//case 1
	{
		...
		return NULL;	//return current page
	}
