...
void replace(unsigned long long *p, 
...
struct cell* insert_in_head(struct list_t* part, long long int page_name, struct cache_t* cache_ptr)
{
	...
	assert(new_head && "make_cell_np return NULL in insert_in_head");
...
struct cache_t* from_mem_to_cache_mem(long long int page_name, struct page_t* mem, struct cache_t* cache_mem)
{
	...
	for (i = 0; i < CACHE_SIZE; i++)				//try to find free space in cache_memory
...
{		
	...
	assert(((T1->length + T2->length) <= CACHE_SIZE) && "Too big real cache");
	...
	assert(((L1_length + L2_length) <= 2 * CACHE_SIZE) && "Too big cache");

	//case 4.1: L1 is full
	if (L1_length == CACHE_SIZE)			
	...
	//case 4.2: if L1 is not full, but together L1+L2 more than cache_size
	else if ((L1_length < CACHE_SIZE) && ((L1_length + L2_length) >= CACHE_SIZE))
	{														
		/*
